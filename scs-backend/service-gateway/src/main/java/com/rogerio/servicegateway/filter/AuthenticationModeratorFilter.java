...
import com.rogerio.servicegateway.util.Constants;
...
import static com.rogerio.servicegateway.util.Constants.MODERATOR;

@Component
public class AuthenticationModeratorFilter extends AbstractGatewayFilterFactory<AuthenticationModeratorFilter.Config> {

    private static Logger log = LoggerFactory.getLogger(AuthenticationModeratorFilter.class);
    ...
    public AuthenticationModeratorFilter() {
        super (Config.class);
    }
        ...
        return ((exchange, chain) -> {
            if(validator.isSecured.test(exchange.getRequest())) {
                ...
                if (response.getStatusCode() == HttpStatus.OK) {
                    if(response.getBody().isStatus() && response.getBody().getRoles().contains(MODERATOR)) {
