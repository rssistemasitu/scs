...
import static com.rogerio.servicegateway.util.Constants.USER;

@Component
public class AuthenticationUserFilter extends AbstractGatewayFilterFactory<AuthenticationUserFilter.Config> {

    private static Logger log = LoggerFactory.getLogger(AuthenticationUserFilter.class);
        ...
        return ((exchange, chain) -> {
            if(validator.isSecured.test(exchange.getRequest())) {
                ...
                if (response.getStatusCode() == HttpStatus.OK) {
                    if(response.getBody().isStatus() && response.getBody().getRoles().contains(USER)) {
